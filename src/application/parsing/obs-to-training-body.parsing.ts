import type { CreateObsToTrainingInput, PatchObsToTrainingInput } from '../ports/obs-to-trainings.port';
...
export function parseObsToTrainingCreateBody(body: unknown): CreateObsToTrainingInput {
  if (!isPlainObject(body)) {
    const err = new Error('Corpo da requisição deve ser um objeto JSON.');
    err.name = VALIDATION;
    throw err;
  }
  if (!('training_id' in body) || !('student_id' in body) || !('obs' in body)) {
    const err = new Error('Campos "training_id", "student_id" e "obs" são obrigatórios.');
    err.name = VALIDATION;
    throw err;
  }
  const obs = expectNullableTrimmedStringOrNull(body.obs, 'obs');
  if (obs === null) {
    const err = new Error('Campo "obs" deve conter texto.');
    err.name = VALIDATION;
    throw err;
  }
  return {
...
export function parseObsToTrainingPatchBody(body: unknown): PatchObsToTrainingInput {
  if (!isPlainObject(body)) {
    const err = new Error('Corpo da requisição deve ser um objeto JSON.');
    err.name = VALIDATION;
    throw err;
  }
  const patch: PatchObsToTrainingInput = {};
  let n = 0;
