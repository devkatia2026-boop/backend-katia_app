...
export class UpdateMyAnamnesisUseCase {
  ...
  async execute(studentId: string, body: unknown): Promise<AnamnesisDTO> {
    ...
    if (Object.keys(values).length === 0) {
      const err = new Error('Informe ao menos um campo para atualizar a anamnese.');
      err.name = VALIDATION;
      throw err;
    }
    ...
    return this.repo.updateLatestForStudent(studentId, values);
  }
